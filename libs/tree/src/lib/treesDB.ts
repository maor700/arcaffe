  ...
  createNewTree = async (
    ...
    return await this.transaction(
      ...
      async () => {
        ...
        const finalTree = { ...new TreeClass(), ...rest, treeName };
        const treeId = await this.trees.add(finalTree);
        addRoot &&
          (await this.addRootNode(treeId as string, treeName || 'root'));
        !treeName &&
          treeId &&
  ...
  duplicateTree = async (treeId: string) => {
    return await this.transaction(
      ...
      async () => {
        ...
        const newTreeId = (await this.createNewTree(
          `${treeName}-copy`,
          false,
          currentTree
        )) as string;
